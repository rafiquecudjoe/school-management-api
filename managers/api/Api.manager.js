...
module.exports = class ApiHandler {
  ...
  /** Map error message strings from entity managers to HTTP status codes */
  _errorCode(msg) {
    if (!msg || typeof msg !== "string") return 400;
  ...
  async _exec({ targetModule, fnName, cb, data }) {
    ...
    try {
      ...
      result.error =
        err && err.message ? err.message : `${fnName} failed to execute`;
    }
    ...
    let hotBolt = this.mwsExec.createBolt({
      ...
      onError: ({ res, error }) => {
        ...
        return this.managers.responseDispatcher.dispatch(res, {
          ok: false,
          code: 500,
          message: error || "middleware stack execution failed",
        });
      },
